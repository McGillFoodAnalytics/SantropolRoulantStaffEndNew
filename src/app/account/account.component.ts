...
export class AccountComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    */
    ...
    if (
      window.location.href.indexOf("apiKey") >= 0 &&
      this.auth.isSignInWithEmailLink(window.location.href)
    ) {
  ...
  open() {
    const modalRef = this.m.open(ManageAccountComponent, {
      size: "md",
      centered: true,
    });
    modalRef.result
  ...
  openAdminSettings() {
    ...
    const modalRef = this.m.open(AdminSettingsComponent, {
      size: "lg",
      centered: true,
    });
    modalRef.result.then((user) => {}).catch((e) => {});
  }
  ...
  async submitWelcome() {
    ...
    const passwordResult = await this.authService.changePassword(
  ...
  loginMessage() {
    this.m
      .open(LoginPopupComponent, { size: "sm", centered: true })
      .result.then(
        (result) => {
          this.submitWelcome();
        },
